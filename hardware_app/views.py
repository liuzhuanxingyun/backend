...
def welcome(request):
    ...
    if res:
        # 修改这里
        img = f"{settings.BACKEND_URL}{settings.MEDIA_URL}{res.img}"
...
def banner_list(request):
    ...
    for b in banners:
        if b.img:
            # 修改这里
            img_url = f"{settings.BACKEND_URL}{settings.MEDIA_URL}{b.img}"
        else:
            img_url = ""
        data.append({
            'id': b.id,
...
def category_list(request):
    ...
    for c in categories:
        # 修改这里
        icon_url = f"{settings.BACKEND_URL}{settings.MEDIA_URL}{c.icon}" if c.icon else ""
        data.append({
            'id': c.id,
            'name': c.name,
            'icon': icon_url
...
def goods_list(request):
    ...
    for g in goods_query:
        # 修改这里
        img_url = f"{settings.BACKEND_URL}{settings.MEDIA_URL}{g.img}" if g.img else ""
        specs = []
...
def goods_detail(request):
    ...
    try:
        ...
        # 修改这里
        img_url = f"{settings.BACKEND_URL}{settings.MEDIA_URL}{goods.img}" if goods.img else ""
...
def tabbar_list(request):
    ...
    for t in tabs:
        # 修改这里
        icon_url = f"{settings.BACKEND_URL}{settings.MEDIA_URL}{t.icon}" if t.icon else ""
        selected_icon_url = f"{settings.BACKEND_URL}{settings.MEDIA_URL}{t.selected_icon}" if t.selected_icon else ""
        data.append({
...
def cart_list(request):
    ...
    for item in items:
        # 修改这里
        img_url = f"{settings.BACKEND_URL}{settings.MEDIA_URL}{item.goods.img}" if item.goods.img else ""
        data.append({
            'id': item.id,
